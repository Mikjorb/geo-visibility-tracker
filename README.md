...
3. **Recommendations** — `POST /api/recommendations` detects gaps rule-based (no AI, no cost)
...
4. **GEO audit** (`/api/geo-audit`) — the "cause-side" technical diagnosis: your site's health for
...
5. **Prompt suggestions** (`POST /api/suggest-prompts`, optional) — turns a site/SEO audit corpus
...
lib/
  ...
  analyze.ts            Mention detection + metrics + canonicalBrand
